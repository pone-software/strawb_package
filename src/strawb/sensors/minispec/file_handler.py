...
class FileHandler:
    def __init__(self, file_name=None,darkcount_cal=False,wavelength_cal=False):
    ...
    def load_meta_data(self):
        ...
        self.module = self.file_name.rsplit('/', 1)[-1].split('_', 1)[0].replace('TUM', '')  
        print(self.module)
    ...
    def calculate_wavelength(self):
        ...
        if self.if_wavecal is True:
            ...
            for n in range(len(self.data_array)):
                ...
                new_wavelength_array = single_device.CAL_ARR[0] + single_device.CAL_ARR[1] * pix + single_device.CAL_ARR[2] * pix ** 2 + \
                                  single_device.CAL_ARR[3] * pix ** 3 + single_device.CAL_ARR[4] * pix ** 4 + single_device.CAL_ARR[5] * pix ** 5
        ...
        for n in range(len(self.data_array)): #n:select a channel
            ...
            for pixel_number in range(self.ADC_shape[1]):
                for measurement_number in range(self.ADC_shape[0]):
                    single_device.ADC_counts_dark[measurement_number:pixel_number] = calibration_obj.darkcounts_fit_function(
...
class spectrum:
    ...
    def __init__(self,input_shape):
        ...
        self.ADC_counts = np.empty(shape=input_shape)  # shape=(n, 288), 288 wavelength pixels, n times of measurements per channel per file
