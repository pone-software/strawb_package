...
wavelength [nm],     A,     B, r**2
"""
bricaud95_para_df = pandas.DataFrame(
...
bricaud95_para_df.sort_values('wavelength', inplace=True)


# Latex notation
# $a^*_{ph}(\lambda) = A(\lambda) \; {chl}^{-B(\lambda)}$
#
# $a_{ph}(\lambda) = A(\lambda) \; {chl}^{-B(\lambda)+1}$
# - $chl$ or $chl\; a$ or $chl_a$: chlorophyll a concentration [$\frac{mg}{m^3}$]
# - $a_{ph}$: absorption coefficient of phytoplankton [$\frac{1}{m}$]
# - $a^*_{ph}(\lambda)$: chl a-specific absorption coefficient of phytoplankton [$\frac{m^2}{mg}$]
#     - $a^*_{ph}(\lambda) = \frac{a_{ph}(\lambda)}{chl}$
#     - alter ver. from text: absorption cross section of algae per mass unit of chl
#     [$ \frac{m^2\; mg}{{chl \; a}^{-1}}$]
...
class Chlorophyll(Absorption):
    config_parameters = bricaud95_para_df  # the default parameters

    def __init__(self, chl=1., config_parameters=None, thickness=1):
    ...
    @property
    def absorption(self):
        """Absorption coefficient [1/m] for the defined wavelengths (config_parameters)"""
        return self.cal_absorption(self.chl)

    def cal_absorption(self, chl):
        """Absorption coefficient [1/m] for the defined wavelengths (config_parameters) and a concentration chl
        PARAMETERS
        ----------
        chl: chlorophyll a concentration in [mg/m^3]
        """
        return self.absorption_polynomial(self.config_parameters.A.to_numpy(),
                                          self.config_parameters.B.to_numpy(),
                                          chl)

    def cal_absorption_chl(self, chl):
        ...
        chl: chlorophyll a concentration in [mg/m^3]
        """
        return self.absorption_polynomial_chl(self.config_parameters.A.to_numpy(),
                                              self.config_parameters.B.to_numpy(),
                                              chl)

    @staticmethod
    def absorption_polynomial(a, b, chl):
        """Polynomial too calculate the absorption coefficient [1/m]
        PARAMETERS
        ----------
    ...
    @staticmethod
    def absorption_polynomial_chl(a, b, chl):
        """Polynomial too calculate the Chl a-specific absorption coefficient of phytoplankton [m^2/mg].
        PARAMETERS
        ----------
        a, b: wavelength dependent parameters
        chl: chlorophyll a concentration in [mg/m^3]
